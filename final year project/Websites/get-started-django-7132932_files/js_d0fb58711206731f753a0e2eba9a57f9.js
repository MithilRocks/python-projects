...
$(document).ready(function() {
  ...
  addEvent(window, "resize", function() {
    ...
    countSize();
//    document.getElementById('container').innerHTML = myWidth;
...
//    console.log('reload');
    ...
    if (myWidth > 768) {
      hideAdvertSizes('mobile'); // possibly should be an array - not for demo purposes
      showOrRefresh('dfp-ad-leaderboard', dfp_leaderboard); // I want desktop so call it
    } else {
      hideAdvertSizes('desktop'); // possibly should be an array - not for demo purposes
      showOrRefresh('dfp-ad-leaderboardmobile', dfp_leaderboardmobile); // I want tablet so call it
    }
  }

  function hideAdvertSizes(size) {
    /*
     Horrible thoughts here : what happens if the advert has bound clicks to the window.top how do I stip that out?
     SKIN'S : we need to understand how to remove them too...
     */

    // size should possibly be an array - but dont forget array.indexOf wont work in < IE7
    if (size == 'desktop') { // case statements are better here.
      document.getElementById('dfp-ad-leaderboard').innerHTML = ''; // kill all the internal content
    } else {
      document.getElementById('dfp-ad-leaderboardmobile').innerHTML = ''; // kill all the internal content
    }
  }
  ...
  function showOrRefresh(advertName, advertVar) {
    ...
    if (shownAds.indexOf(advertName) == -1) { // GOTCHA - IE7 will not work here...
      ...
      if (document.getElementById('dfp-ad-leaderboard').innerHTML == '') {
        googletag.pubads().refresh([advertVar]);
        console.log('refreshed ' + advertName);
      }
    }
